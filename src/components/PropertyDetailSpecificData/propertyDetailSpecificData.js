...
export const PropertyDetailSpecificData = (props) => {
  ...
  var array_of_specific_data = mapObject(specificData, (keyname, value) => ({
    name: keyname,
    value,
  }));
  array_of_specific_data = array_of_specific_data.filter(
    (item) => item.value !== 0
  );
              ...
              <Box p={1}>
                <Grid container justifyContent="center" alignItems="center">
                  <Grid item xs={6}>
                    ...
                    >
                      {features[item.name] ? features[item.name].name : ""}
                    </Typography>
                  </Grid>
                  <Grid item xs={6}>
                    <Typography
