...
import { unlinkPhone, registerOrLoginWidthGoogle, register } from '../../firebase/user'
...
export function Register(props) {
  ...
  */
  ...
  const { handleChange, handleBlur, Errors, Values, setValues, setErrors } = useFormValidation(initialState, validateReg)
  ...
  const handleChangeShowMainPhone = event => {
    ...
    if (extraPhones.length === 0) return setAlert(null, ' ', 'Primero agrega otro número de teléfono', 'error')
    setAlert(
  ...
  const handleRemoveErrorExtraNumber = event => {
    ...
    setExtraPhoneErrors(extraPhoneErrors.filter((error) => error.toString() !== Currentindex.toString()))
  }
  ...
  const handleDeleteExtraPhones = (indexToDelete) => {
    setExtraPhoneErrors([])
    const newExtraPhones = extraPhones.filter((phone, index) => index !== indexToDelete)
  ...
  const verificateSMSCode = async code => {
    try {
      ...
      console.log('verificateSMSCode', status)
      return  status
    } catch (error) {
      console.log('verificateSMSCode', error)
      const errorStatus = error.code === 'auth/credential-already-in-use' ? 'Este número ya se encuentra registrado' : 'Código no válido'
  ...
  const createOrValidate = async () => {
    setLoading(true);
    if (Object.keys(Errors).length === 0) { //no error 
      if (Values.email.length !== 0
        ...
        if (props.federated) { //comes from loggin
  ...
  const handleLogInG = async () => {
    ...
    console.log(response, Values, 'debuging')
    ...
    if (response.success && response.isComplete) {
      ...
      setAlert( null, ' ', 'El correo ya se encuentra registrado, incia sesión nuevamente', 'error')
      props.setFederated(false)
    } else {
      props.setFederated(false)
    }
  ...
  const setPromoter = () => {
    setValues({ ...Values, promoter: true })
  }

  const setClient = () => {
    setValues({ ...Values, promoter: false })
  }
  ...
  const handlePromoterTypeChange = ({ companyName, promoterType }) => {
    console.log('companyName, promoterType', companyName, promoterType)
    setValues({
      ...Values,
      companyName,
      promoterType
  ...
  return (
    <React.Fragment>
      <Grid container justifyContent='center' alignItems='center' spacing={2} style={{maxHeight: '85vh'}}>
        ...
        {typeof(Values.promoter) !== 'string' && !loading && (!!Values.promoterType || Values.promoter === false) && (
          <Form
        ...
        {Values.promoter === 'none' && (
          <Grid item xs={12}>
            <Typography variant='subtitle1' align='center' style={{fontWeight:'bold', color: 'gray'}}>
              ¿Ya tienes cuenta?<SessionButton onClick={props.changeToLogIn}> Inicia Sesión </SessionButton>?
            </Typography>
          </Grid>
        )}
      </Grid>
