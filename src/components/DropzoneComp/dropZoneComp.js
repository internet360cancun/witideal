...
export function DropZoneComp(props) {
  ...
  const { getRootProps, getInputProps, isDragActive } = useDropzone({
    ...
    onDrop: (acceptedFiles, rejectedFiles) => {
      let numFilesInSection =
        props.files[props.section] !== undefined
          ? Object.entries(props.files[props.section]).length
          : 0;
      let availableMemory =
        props.maxPhotos - (numFilesInSection + props.imgUrls.length);
      if (acceptedFiles.length <= availableMemory) {
        keepFiles(acceptedFiles);
      } else {
        handleWarning(
          `Aún dispones de ${availableMemory} espacios para fotos, pero estás intentando subir ${acceptedFiles.length} fotos.`
        );
      }
  ...
  const dragInactive =
    Object.entries(
      props.files[props.section] !== undefined ? props.files[props.section] : []
    ).length <
    props.maxPhotos - props.imgUrls.length ? (
      <React.Fragment>
        <Grid item xs={12}>
